...
//Using M_PI hence not actual calculation, since using pi to calculate pi
#include <iostream>
#include <cmath>
#include <cstdlib>
...
float montecarlosim(int N, float t, float l) {
	...
	float theta[N]; //array to store random theta for every throw
	int counter=0;
	
	for (int i=0; i<N; i++) {
		x[i]=(t/2)*(float)rand()/RAND_MAX; //generating random values of x between 0 and t/2
		theta[i]=(M_PI/2)*(float)rand()/RAND_MAX; //generating random values of theta between 0 and pi/2
		
		if (x[i]<=l*sin(theta[i])/2) {
